...
def test_create_user():
    ...
    response = client.post('/users', json={"email": f"ada+{rnd}@test.com", "password": "password"})
    if not response.ok:
        print(response.text)
        assert False
...
def test_create_incident_for_user():
    ...
    response = client.post(f'/users/{user_id}/incidents', json={'title': 'new incident for ada'})
    assert response.status_code == 201
    incident_id = response.json()['id']
    assert response.json() == {'title': 'new incident for ada',
                               'description': None,
                               'id': incident_id,
                               'owner_id': user_id}
